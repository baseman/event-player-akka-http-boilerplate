...
@Path("/shoppingcart")
@Api(value = "shoppingcart" /* ,authorizations = [Authorization(value = "sampleoauth", scopes = [])]*/)
@Produces("application/json")
class ShoppingCartDtoRouter(private val dtoActor: ActorRef, val timeout: Timeout) : AllDirectives() {
    @GET
    @Path("/item/{rootId}")
    @Produces("application/json")
    ...
    @ApiResponses(value = [
        (ApiResponse(code = 200, response = ShoppingCartRoot::class, message = "item results")),
        ...
        pathPrefix("item") {
                ...
                path<String>(PathMatchers.segment()) { rootId ->
                    ...
                    val returnItem = PatternsCS.ask(
                            dtoActor,
                            RootDtoMessages.GetItem<ShoppingCartRoot>(RootId(rootId)),
                            timeout
                    ).thenApply { obj -> obj as RootDtoMessages.ReturnItem<ShoppingCartRoot> }

                    onSuccess<RootDtoMessages.ReturnItem<ShoppingCartRoot>>({ returnItem }, { result ->
                        when {
                            result.item == null -> complete("")
                            else -> complete<ShoppingCartRoot>(StatusCodes.OK, result.item, Jackson.marshaller())
                        }
                    })
...
//            }
    ...
    @ApiResponses(value = [
        (ApiResponse(code = 200, response = Array<ShoppingCartRoot>::class, message = "item list results"))
    ])
    fun getRoute(): Route? {
